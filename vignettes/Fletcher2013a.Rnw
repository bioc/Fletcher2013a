...
A pre-processed differential expression dataset is also available for the function \Rfunction{Fletcher2013pipeline.deg}, which can be used to retrieve the DE genes inferred in the limma analysis, as for example:
